...
public class DatabaseService extends Service {
    ...
    protected static final int SEND_BIKE_DATA = 1;
...
//        startMBandCaloriesDataSync();
        ...
        // create an object for storing command sent
        commandSent = "0";
    ...
    private void registerDatabaseReceiver() {
        filter = new IntentFilter("ie.ucd.smartride.database");
...
//            client = BandClientManager.getInstance().create(getBaseContext(), pairedBands[0]);
    ...
    private class ProcessBikeDataThread extends Thread{
        ...
        public void run(){
            ...
            //need to have a god damn array to send to another class using Parcelable (unless BikeData class is set to implement Parcelable, which it isn't)
            float[] bikeDataFloatArray = {voltage, current, torque, rpm, humanPower};

            //use broadcast receiver to reduce number of calls to database
            mHandler.obtainMessage(SEND_BIKE_DATA, bikeDataFloatArray).sendToTarget();
    ...
    Handler mHandler = new Handler(new Handler.Callback() {
        ...
        public boolean handleMessage(Message msg) {
            ...
            switch (msg.what) {
                case SEND_BIKE_DATA:
                    float[] bikeDataFloatArray = (float[]) msg.obj;
                    Intent bikeData_intent = new Intent();
                    bikeData_intent.addFlags(Intent.FLAG_INCLUDE_STOPPED_PACKAGES);
                    bikeData_intent.putExtra("bikeData", bikeDataFloatArray);
                    //bikeData_intent.putExtra("bikeData", bikeData);
                    bikeData_intent.setAction("ie.ucd.smartride.bikeData");
                    sendBroadcast(bikeData_intent);
                    break;
