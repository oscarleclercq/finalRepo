...
public class MapsActivity extends FragmentActivity implements OnMapReadyCallback {
    ...
    public void onMapReady(GoogleMap googleMap) {
        ...
        //LatLng user = new LatLng(37.42, -122.08);

        //target is the target spot to make a circular geofence around, change this for new spots

        LatLng target = new LatLng(51.49913, -0.17428);
        addMarker(target);
...
// have to add more stuff here if i want to use api level 29+ ref vid
        enableUserLocation();
    }
    ...
    public void addGeofence(LatLng latLng, float radius) {
        // This is where I set it to trigger on enter rather than exit or dwell
        Geofence geofence = geofenceHelper.getGeofence(GEOFENCE_ID, latLng, radius, Geofence.GEOFENCE_TRANSITION_ENTER | Geofence.GEOFENCE_TRANSITION_DWELL | Geofence.GEOFENCE_TRANSITION_EXIT);
        GeofencingRequest geofencingRequest = geofenceHelper.getGeofencingRequest(geofence);
        PendingIntent pendingIntent = geofenceHelper.getPendingIntent();

        // might be an issue with listeners below, not italicised and unused vs aVoid
